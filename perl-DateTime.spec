...
BuildRequires:  %{?scl_prefix}perl(DateTime::Locale) >= 1.06
BuildRequires:  %{?scl_prefix}perl(DateTime::TimeZone) >= 2.02
...
BuildRequires:  %{?scl_prefix}perl(Params::ValidationCompiler) >= 0.26
BuildRequires:  %{?scl_prefix}perl(parent)
BuildRequires:  %{?scl_prefix}perl(POSIX)
BuildRequires:  %{?scl_prefix}perl(Scalar::Util)
BuildRequires:  %{?scl_prefix}perl(Specio) >= 0.18
BuildRequires:  %{?scl_prefix}perl(Specio::Declare)
BuildRequires:  %{?scl_prefix}perl(Specio::Exporter)
BuildRequires:  %{?scl_prefix}perl(Specio::Library::Builtins)
BuildRequires:  %{?scl_prefix}perl(Specio::Library::Numeric)
...
BuildRequires:  %{?scl_prefix}perl(DateTime::Format::Strptime)
...
%global __requires_exclude %{?__requires_exclude:%__requires_exclude|}^%{?scl_prefix}perl\\((DateTime::Locale|DateTime::TimeZone)\\)$
...
%build
%{?scl:scl enable %{scl} '}perl Makefile.PL \
  INSTALLDIRS=vendor \
  OPTIMIZE="%{optflags}" \
  NO_PACKLIST=1 \
  NO_PERLLOCAL=1 && %{make_build}%{?scl:'}
...
%changelog
* Mon Jan 06 2020 Jitka Plesnikova <jplesnik@redhat.com> - 2:1.51-5
- SCL
...
- Use NO_PERLLOCAL=1 so we can use "%{?scl:scl enable %{scl} '}make install"%{?scl:'}
